...
export async function POST(req: NextRequest) {
  try {
    ...
    // ============================================
      ...
      const indexResult = await waitForPipelineFilesIndexing(pipelineId, 300000) // 5 minutes for larger PDFs
    ...
    try {
      ...
      console.log(`✅ Extraction metadata saved for user ${userId}`)
    } catch (dbError) {
      console.error("Failed to save extraction metadata:", dbError)
